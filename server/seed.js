...
import { db, all, get, run, tx, nowStr } from './lib/db.js';
...
tx(() => {
  ...
  ]) run('INSERT INTO users (username, full_name, role, password_hash) VALUES (?,?,?,?)', username, name, role, hashSecret(pw));
  ...
  ]) run('INSERT INTO warehouses (wh_code, wh_name, address, grid_cols, grid_rows) VALUES (?,?,?,?,?)',
    code, name, address, cols, rows);

  const whId = Object.fromEntries(all('SELECT wh_code, warehouse_id FROM warehouses').map((w) => [w.wh_code, w.warehouse_id]));
  ...
  ]) run('INSERT INTO zones (zone_code, zone_name, warehouse_id, color) VALUES (?,?,?,?)', code, name, whId[wc], color);

  const zoneId = Object.fromEntries(all('SELECT zone_code, zone_id FROM zones').map((z) => [z.zone_code, z.zone_id]));
  ...
  ]) run('INSERT INTO rags (rag_no, zone_id, total_levels, total_depths, pos_x, pos_y) VALUES (?,?,?,?,?,?)',
  ...
  ]) run('INSERT INTO skus (sku_code, sku_name, category, unit, barcode) VALUES (?,?,?,?,?)',
    code, name, cat, unit, '885' + String(1000000 + Math.floor(rnd() * 8999999)));
});

for (const r of all('SELECT rag_id FROM rags')) syncRagLocations(r.rag_id);

// ปิดใช้งานบางตำแหน่ง (จำลองช่องที่ชำรุด)
run("UPDATE locations SET status='DISABLED' WHERE location_code IN ('FG-A01-L4-D5','FG-A01-L4-D6')");

// จัดเก็บสินค้าตัวอย่าง
const skus = all('SELECT * FROM skus');
const staff = get("SELECT user_id FROM users WHERE username='staff'").user_id;
...
tx(() => {
  for (const loc of all(
    ...
    run("UPDATE locations SET status='OCCUPIED' WHERE location_id = ?", loc.location_id);
    run(`INSERT INTO movements (movement_type, item_id, sku_id, lot_no, quantity, to_location_id, user_id, note)
         VALUES ('STORE',?,?,?,?,?,?,'ข้อมูลตัวอย่างเริ่มต้นระบบ')`,
      Number(res.lastInsertRowid), sku.sku_id, `2569-${lot}`, qty, loc.location_id, staff);
  }
});

console.log(`ตำแหน่งจัดเก็บทั้งหมด ${get('SELECT COUNT(*) AS n FROM locations').n} ตำแหน่ง`);
console.log(`มีสินค้าอยู่ ${get("SELECT COUNT(*) AS n FROM locations WHERE status='OCCUPIED'").n} ตำแหน่ง`);
console.log('\nบัญชีทดสอบ:  admin/admin123 · staff/staff123 · viewer/viewer123');
if (process.argv[1]?.endsWith('seed.js')) db.close();
