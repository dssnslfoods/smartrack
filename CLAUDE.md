...
## AI Layer (เพิ่มเมื่อ ส.ค. 2569)

แยกเป็น 2 ชั้นชัดเจน เพื่อให้ตัวเลขตรวจสอบย้อนกลับได้เสมอ:

| ชั้น | ไฟล์ | ทำอะไร | ต้องมี API key? |
|---|---|---|---|
| สมองวิเคราะห์ | `server/services/intelligence.js` | คำนวณด้วย SQL ล้วน | ❌ ไม่ต้อง |
| ชั้น AI | `server/services/ai.js` + `server/lib/claude.js` | เรียก Claude API เรียบเรียงเป็นภาษาคน | ✅ ต้องมี |

**หลักการที่ห้ามละเมิด:**
1. **AI เขียนข้อมูลไม่ได้** — tool ทุกตัวที่ AI เรียกได้เป็น read-only ทั้งหมด สิ่งที่ AI เสนอ
   (บรรทัด GRN จากรูป, ใบโอนย้าย, จำนวนที่นับได้) ต้องให้คนตรวจและกดยืนยันเองเสมอ
2. **ตัวเลขห้ามมาจาก AI** — ทุกตัวเลขต้องผ่าน `intelligence.js` หรือ service เดิม
   AI มีหน้าที่เรียบเรียงเท่านั้น ห้ามให้ AI คำนวณเอง
3. **ไม่มี key ต้องยังใช้งานได้** — หน้า AI Insights แสดงตัวเลขได้ปกติ แค่ไม่มีคำแนะนำสรุป

**ฟีเจอร์ที่ทำแล้ว:**
- `1.1` ผู้ช่วยถาม-ตอบ (`#/copilot`) — Claude tool use เรียก 10 เครื่องมืออ่านข้อมูล
- `1.2` สแกนใบส่งของ (ในหน้า GRN) — Claude Vision อ่านเอกสาร → เติมฟอร์มให้ตรวจ
- `1.3` Expiry Intelligence — เทียบอัตราจ่ายออกจริงกับคิว FEFO หา Lot ที่จะขายไม่ทัน
- `2.1` Smart Slotting — ABC analysis + เสนอสลับ/ย้ายตำแหน่ง ลดระยะเดินหยิบ
- `2.2` Demand Forecast — ค่าเฉลี่ยถ่วงน้ำหนัก + แนวโน้ม → จุดสั่งเติม
- `2.3` Anomaly Detection — นับไม่ตรงซ้ำ · แก้ไขถี่ผิดปกติ · งานค้าง · ข้าม FEFO · ปริมาณผิดปกติ
- `3.1` Vision Counting (ในหน้านับสต็อก) — ถ่ายรูปชั้นวาง → AI ช่วยประมาณจำนวน
- `3.2` Labor Planning — ภาระงานตามวัน/ชั่วโมง → ประมาณจำนวนคนที่ควรจัดกะ

**ต้นทุนเข้าถึงตำแหน่ง (ใช้ใน Smart Slotting):** `(level−1)×3 + (depth−1)×2` — ยิ่งน้อยยิ่งหยิบง่าย
สอดคล้องกับกฎ Drive-in rack: D1 หน้าสุดเข้าถึงตรง, L1 ชั้นล่างสุดหยิบง่ายที่สุด

**API:** `/api/insights/*` (คำนวณล้วน) · `/api/ai/*` (ต้องมี key) — ดู `server/api/routes.js`
